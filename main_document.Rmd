...
downloadSummaryOfImmediateNotificationsAndFollowUps <- function(disease_id_hidden,
                                                                disease_type_hidden,
                                                                year
                                                                ) {
        ...
        if(!diseaseID_Type %in% diseaseIDs_Types)
                stop(paste0("The entered combination of disease_id_hidden and disease_type_hidden (\"",
                            diseaseID_Type,
                            "\") doesn't exist."))
...
parseSummaryOfImmediateNotificationsAndFollowUps <- function(nonParsedSummary) {
        if(!all(c("year",
                  "disease_id_hidden",
                  "disease_type_hidden",
                  "resp") %in% names(nonParsedSummary)) |
                   !"response" %in% class(nonParsedSummary[["resp"]]))
                stop ("The argument specified is not a list containing a response object.")
        response <- nonParsedSummary[["resp"]]
        url <- "http://www.oie.int/wahis_2/public/wahid.php/Diseaseinformation/Immsummary"
        ...
        year <- nonParsedSummary[["year"]]
        disease_id_hidden <- nonParsedSummary[["disease_id_hidden"]]
        disease_type_hidden <- nonParsedSummary[["disease_type_hidden"]]
...
getDiseaseSummaries <- function(formValues,
        ...
        attr(summaries, "countries") <- countries
...
downloadSummaryOfOutbreaks <- function(summaryRecord) {
        ...
        resp <- POST(url = url,
                     body = paste0("reportid=",
                                   summaryRecord[["reportid"]],
                                   "&summary_country=",
                                   summaryRecord[["summary_country"]]),
                     content_type("application/x-www-form-urlencoded"),
...
parseSummaryOfOubreaks <- function(x) {
        if(!all(c("resp") %in% names(x)) |
                   !"response" %in% class(x[["resp"]]))
                stop ("The argument specified is not a list containing a response object.")
        url <- "http://www.oie.int/wahis_2/public/wahid.php/Diseaseinformation/Immsummary/listoutbreak"
        postfields <- rawToChar(x[["resp"]]
...
getSummariesOfOutbreaks <- function(diseaseSummaries) {
        ...
        message(paste0("Downloading summaries of outbreaks (",
                       ...
                       "):"))
        summaries <- apply(diseaseSummaries, 1, downloadSummaryOfOutbreaks)
